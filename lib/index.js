#!/usr/bin/env sh
':' //; exec "$(command -v "`dirname $0`/node" || command -v node)" "--no-warnings" "$0" "$@"
...
function mdon(pkgpath = './package.json', mdpath = './README.md', outpath = true) {
...
const
    argi = process.argv.findIndex(arg => /index.m?js$/.test(arg)),
