const defaults = { backup: false, safe: true, output: true }, debugging = '';
const fs = require('fs'), path = require('path'), { assign: define, entries: entriesOf, getPrototypeOf, setPrototypeOf: setPrototype } = Object, { readFileSync, existsSync, writeFileSync, renameSync } = fs, { resolve, dirname, basename, relative, parse: parsePath, extname } = path;
...
;
const VOID = Object.seal(Object.create(null)), NOOP = (() => VOID), ANY = (type) => type !== 'undefined', prototypeOf = (value) => value === null || value === undefined ? undefined : getPrototypeOf(value), typeguard = (type, value, fallback) => typeof value === type ? value : fallback, callable = typeguard.bind(null, 'function'), object = define(typeguard.bind(null, 'object'), {
    flat: ((check = Array.prototype.includes.bind([null, Object.prototype])) => (value, fallback) => check(prototypeOf(value)) ? value : fallback)()
}), boolean = typeguard.bind(null, 'boolean'), string = typeguard.bind(null, 'string'), { stdout, argv = [], hrtime } = ANY(typeof process) ? process : VOID, columns = stdout && stdout.columns > 0 ? Math.min(stdout.columns, 120) : 80, { now = (callable(hrtime) && ((t = hrtime()) => t[0] * 1000 + t[1] / 1000000)) || Date.now } = ANY(typeof performance) ? performance : VOID, bind = (object, ...methods) => {
    for (const method of methods)
        callable(object[method]) && (object[method] = object[method].bind(object));
}, reconcile = ((a, ...fallback) => {
    try {
...
function mdon(pkgpath = '', mdpath = './README.md', outpath = defaults.output) {
    ...
    let frontmatter = [];
    const properties = {}, append = yaml.safeLoad ? (matter) => string(matter) && (matter = matter.replace(/^(?:<\!)?---\n((?:.*?\n)*?|)---(?:>)?\n?$/, '$1')) && (define(properties, yaml.safeLoad(matter)), frontmatter.push(matter), true) : NOOP;
    const firstLine = pkg.readUntil(mdpath.path, /(?:([\?\@\!\%])(--)?>)/) || '';
    let [, extramatter = '', rawpath] = /(<\!---\n(?:.*?\n)*?--->\n|---\n(?:.*?\n)*?---\n|)(?:.*?\n)*?(?:<\!--[\@\%])(.*?)$/.exec(firstLine) || '';
    if (extramatter)
        extramatter = extramatter.replace(/^<\!/, '').replace(/>\n$/, '\n');
    ...
    const context = new Context({ ...info, ...properties, [READ]: read, [PARSE]: parse, document: parsePath(mdpath.resolved), src: mdpath.resolved }, root);
    ...
    if (outmatter && mdpath.raw)
        outmatter = `<!${outmatter.replace(/[\s\n]+$/, '').trim()}>\n`;
